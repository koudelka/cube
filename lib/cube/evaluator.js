...
exports.register = function(db, endpoints) {
  var event = require("./event").getter(db),
      ...
      distinct = require("./distinct").getter(db);
  ...
  endpoints.ws.push(
    ...
    endpoint("/1.0/distinct/get", distinct)
  );
  ...
  endpoints.http.push(
    ...
    endpoint("GET", "/1.0/distinct", distinctGet),
    endpoint("GET", "/1.0/distinct/get", distinctGet)
  );
  ...
  function distinctGet(request, response) {
    distinct(url.parse(request.url, true).query, function(data) {
      response.writeHead(200, headers);
      response.end(JSON.stringify(data));
    });
  }
